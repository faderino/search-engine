...
describe("NewsCard", () => {
  ...
  test("should able to toggle sub articles", () => {
    render(<NewsCard news={news} />);

    const toggleSubNews = screen.getByTestId("toggle-sub-news");
    userEvent.click(toggleSubNews);
    const subNews = screen.getByRole("list");

    expect(subNews).toBeInTheDocument();
  });

  test("should not render toggle button when there is no sub articles", () => {
    render(<NewsCard news={{ ...news, sub_articles: [] }} />);

    const toggleSubNews = screen.queryByTestId("toggle-sub-news");

    expect(toggleSubNews).not.toBeInTheDocument();
  });

  describe("Bookmark button clicked", () => {
    const addToReadingList = jest.fn();
    const removeFromReadingList = jest.fn();

    test("should add to reading list", () => {
      render(<NewsCard news={news} addToReadingList={addToReadingList} />);

      const bookmarkButton = screen.getByTestId("BookmarkBorderOutlinedIcon");
      userEvent.click(bookmarkButton);

      expect(addToReadingList).toHaveBeenCalledTimes(1);
      expect(addToReadingList).toBeCalledWith(news);
    });
    ...
    test("should remove from reading list", () => {
      ...
      const removeBookmarkButton = screen.getByTestId("BookmarkIcon");
      userEvent.click(removeBookmarkButton);

      expect(removeFromReadingList).toHaveBeenCalledTimes(1);
